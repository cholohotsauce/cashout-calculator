...
// === Google Drive Integration using Redirect-Based OAuth ===
...
function initClient() {
  gapi.client.init({
    ...
    // Listen for sign-in state changes (for logging purposes)
...
function createFileInDrive(reportText) {
  const fileMetadata = {
    ...
    'parents': ['1Q8cre1aJIP2BTdS1ZUjyogx5DfMBpVov'] // "Cash-out reports" folder ID
  };
  const media = {
    mimeType: 'text/plain',
    body: reportText
...
// Google Drive Button event listener using redirect-based sign-in
document.getElementById("googleDriveBtn").addEventListener("click", () => {
  ...
  if (!authInstance.isSignedIn.get()) {
    console.log("User is not signed in. Initiating sign-in with redirect...");
    // Initiate sign-in using redirect mode
    authInstance.signIn({
      ux_mode: 'redirect',
      redirect_uri: 'https://cholohotsauce.github.io/the-ace-cocktail-finder/'
    }).catch(err => {
